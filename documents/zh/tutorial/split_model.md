...
主页面模型(`TodoList`)中的查询条件必须通过提交按钮，使其生效改变，而在一次查询完毕后，为了保持查询条件和显示列表的一致性，还需要用生效查询条件对显示查询条件进行清理重置。这才是一个完整经典查询页面对条件的处理方案。所以，我们需要提取一个`SearchParamsModel`新模型专门为基础条件显式变更服务。

源码位置：[splitModel.spec.ts](https://github.com/filefoxper/agent-reducer/blob/master/test/zh/tutorial/splitModel.spec.ts).
...
```
目前为止，我们的查询页面模型已经可以很好得工作了，让我们对它进行一些更酷的加工，[下一节](https://github.com/filefoxper/agent-reducer/blob/master/documents/zh/tutorial/take_latest.md)使用`MiddleWarePresets.takeLatest`。