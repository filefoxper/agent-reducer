...
异步请求数据并不总是按请求顺序逐一返回的，有时请求会被网络延时等突发情况耽搁。这可能导致一个早前发起的请求响应数据在最新响应数据渲染后返回并覆盖最新数据。这时候我们可以使用`MiddleWarePresets.takeLatest`来避免这个问题。

源码位置：[takeLatest.middleWare.spec.ts](https://github.com/filefoxper/agent-reducer/blob/master/test/zh/tutorial/takeLatest.middleWare.spec.ts).
...
```
如果你希望在快速翻页时，使用 debounce 特性，你可以继续添加 MiddleWare，[下一节](https://github.com/filefoxper/agent-reducer/blob/master/documents/zh/tutorial/complex.md)更复杂的 MiddleWare 特性。