# use more complex middleWare

When we are changing currentPage, a fetching request is started, if we can use debounce feature on `changePage` method, the request frequency will be throttled down.

You can check code in [complex.middleWare.spec.ts](https://github.com/filefoxper/agent-reducer/blob/master/test/en/tutorial/complex.middleWare.spec.ts).
...
You can do more things to make a search page model better. Let us learn more about `agent-reducer`, [see the guides](https://github.com/filefoxper/agent-reducer/blob/master/documents/en/guides/about_this.md).