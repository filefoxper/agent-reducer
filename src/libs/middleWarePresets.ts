...
export class MiddleWarePresets {
    ...
    static takeThrottle(wait:number){
        return applyMiddleWares(MiddleWares.takeThrottle(wait), MiddleWares.takePromiseResolve());
    }

    static takeDebounce(wait:number,opt?:{leading?:boolean}){
        return applyMiddleWares(MiddleWares.takeDebounce(wait,opt), MiddleWares.takePromiseResolve());
    }
    ...
    static takeThrottleAssignable(wait:number){
        return applyMiddleWares(MiddleWares.takeThrottle(wait), MiddleWares.takePromiseResolve(),MiddleWares.takeAssignable());
    }

    static takeDebounceAssignable(wait:number,opt?:{leading?:boolean}){
        return applyMiddleWares(MiddleWares.takeDebounce(wait,opt), MiddleWares.takePromiseResolve(),MiddleWares.takeAssignable());
    }
