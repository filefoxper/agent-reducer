...
如果你想要了解如何优化 `agent-reducer` 在应用中的体积占比，请继续当前段落，否则您可以直接进入 [入门](/zh/introduction?id=入门) 阶段。

为了帮助应用减小库文件的体积，我们提供了 `agent-reducer/es` 源码包。应用可通过使用 [babel](https://babeljs.io/docs/en/configuration) 自行编译。
...
在使用源码包的过程中，如果你依然希望使用 `import {...} from 'agent-reducer'` 来引用 API，可以尝试使用 `alias` 别名系统。

以 webpack.config.js 为例:
