import {
  createAgentReducer,
  middleWare,
  MiddleWares, NextProcess,
  OriginAgent, Runtime, StateProcess,
} from '../src';
...
describe('how to use createAgentReducer', () => {
  ...
  const throughMiddleWare = (runtime: Runtime) => {
    return (next: StateProcess): NextProcess => {
      return (result: any): any => {
        if (result === undefined || result === null) {
          return next(runtime.target.state);
        }
        return next(result);
      }
    }
  ...
  it('use env directly on createAgentReducer', () => {
    // createAgentReducer has a simple reducer processor inside.
